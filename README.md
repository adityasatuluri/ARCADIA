...
- **Zero-Config Xbox Emulation:** The built-in scanner automatically detects `xemu` and dynamically injects the necessary ROM and HDD arguments at launch time (e.g. `-bootrom`, `-flash`, `-hdd`, `-dvd_path`), bypassing the need to ever touch internal emulator settings.
...
3. **Build and Package:**
   ...
   npm run package
   ```
   *This compiles the React app into `dist/` and runs `electron-builder` to create a standalone, portable `Arcadia.exe` in the `release/` directory.*
